    ...
    for line in range(len(lines)):
        ...
        for field in range(1, len(this_line)):
            if line >= 2:
                ...
                if field == d_connect and this_line[d_connect] != "":
                    text = "{} {}'s Davidson Connect: {}; ".format(this_line[fname], this_line[lname], this_line[d_connect])
                    notes += text
                if field == social  and this_line[social] != "":
                    text = "{} {}'s Social media: {}; ".format(this_line[fname], this_line[lname], this_line[social])
                    notes += text
                if field == quote and this_line[quote] != "":
                    text = "{} {}'s Quote/Tagline: {}".format(this_line[fname], this_line[lname], this_line[quote])
                    notes += text

                if field == linkedin and this_line[linkedin] != "":
                    if field == website and this_line[website] != "":
                        # print(this_line[website])
                        main += website_field(this_line[website])
                        vc = vcard(this_line[fname], this_line[lname], this_line[email], this_line[cell], this_line[website], notes)        	
                    else:
                        # print(this_line[linkedin])
                        main += website_field(this_line[linkedin])     
                        vc = vcard(this_line[fname], this_line[lname], this_line[email], this_line[cell], this_line[linkedin], notes)        	
                else:
                    vc = vcard(this_line[fname], this_line[lname], this_line[email], this_line[cell], this_line[website], notes)        	

        main += str(notes_field(notes))
...
def vcard(fname, lname, email, phone, website, note):
    name = fname + " " + lname
    cell_filter = filter(str.isdigit, phone)
    cell = "".join(cell_filter)
...
NOTE: {}
END:VCARD""".format(lname, fname, name, email, cell, website, note)
...
def main():
    filename = "/Users/izzymoody/Desktop/WLC/digital-namecard-master/FormSubmissions.csv"
